<template>
  <view class="login-page">
    ...
    <view class="card">
      ...
      <view class="switch" @click="goRegister">
        <text class="switch-text">还没有账号？立即注册</text>
      </view>
    </view>
  </view>
</template>
...
<script setup>
// 登录页：调用 /auth/login，成功后将 token 与用户信息写入本地登录态，跳转计划列表。
// 合约依据：api/auth.js → POST /auth/login
import { ref } from 'vue'
...
function submit() {
  ...
  login({ username: u, password: p })
    ...
    .catch(err => {
      // 登录接口已 silent:true，错误文案来自后端 401 的 detail
      errorMsg.value = (err && err.message) || '账号或密码错误'
    })
}
